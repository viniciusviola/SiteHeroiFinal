...
const updateCardsQuantity = () => {
  document.getElementById("cards-quantity").textContent = heroDeck.length
}

const drawCard = () => {
  const computerCardNumber = Math.floor(Math.random() * (heroDeck.length - 1));
  computerCard = heroDeck[computerCardNumber];
  heroDeck.splice(computerCardNumber, 1);


  const playerCardNumber = Math.floor(Math.random() * (heroDeck.length - 1));
  playerCard = heroDeck[playerCardNumber];
  heroDeck.splice(playerCardNumber, 1);
